...
@section('main_content')
    <div class="page-content">
        ...
        <!--end breadcrumb-->
        <h6 class="mb-0 text-uppercase">Driver Details List</h6>
        <hr />
        <div class="card">
            <div class="card-body">
                <div class="table-responsive">
                    <table id="example2" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach ($driverData as $driverData_Val)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $driverData_Val->first_name }} {{ strtoupper(substr(trim($driverData_Val->last_name), 0, 1)) }}</td>
                                    <td>{{ $driverData_Val->email ?? '-' }}</td>
                                    <td>{{ $driverData_Val->country_name ?? '-' }}</td>
                                    <td>{{ $driverData_Val->state_name ?? '-' }}</td>
                                    <td>{{ $driverData_Val->district_name ?? '-' }}</td>
                                    <td>{{ $driverData_Val->city_name ?? '-' }}</td>
                                    <td>
                                        {{ $driverData_Val->area_name ?? '-' }}
                                        @if ($driverData_Val->pincode)
                                            - {{ $driverData_Val->pincode }}
                                        @endif
                                    </td>
                                    <td>{{ $driverData_Val->area_manager_name }}</td>
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="form-check form-switch m-0">
                                                <input class="form-check-input" type="checkbox" role="switch"
                                                    id="statusSwitch{{ $driverData_Val->id }}"
                                                    data-id="{{ $driverData_Val->id }}"
                                                    {{ $driverData_Val->status ? 'checked' : '' }}
                                                    onchange="updateStatus(this)">
                                            </div>
                                            {{-- <span id="status-text-{{ $driverData_Val->id }}"
                                                class="form-check-label status-badge {{ $driverData_Val->status ? 'status-active' : 'status-inactive' }}">
                                                {{ $driverData_Val->status ? 'ACTIVE' : 'INACTIVE' }}
                                            </span> --}}
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <a href="javascript:void(0)" class="text-primary me-2 viewAreaManager"
                                                data-id="{{ $driverData_Val->id }}" title="View">
                                                <i class="bx bx-show fs-5"></i>
                                            </a>

                                            <!-- Edit -->
                                            <a href="{{ route('admin.driver.edit', $driverData_Val->id) }}"
                                                class="text-primary" title="Edit">
                                                <i class="bx bx-edit fs-5"></i>
                                            </a>

                                            <!-- Delete -->
                                            <form action="{{ route('admin.driver.destroy', $driverData_Val->id) }}"
...
<script>
    function updateStatus(checkbox) {
        ...
        $.ajax({
            url: "{{ route('admin.driver.update_status') }}",
            type: 'POST',
            data: {
                id: id,
                status: status
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('submit', '.delete-form', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'You want to delete this driver details?',
                icon: 'warning',
...
<script>
    $(document).on('click', '.viewAreaManager', function() {
        ...
        $.ajax({
            ...
            url: "{{ url('admin/driver-details') }}/" + id + "/view",
